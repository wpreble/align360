...
## Close new signups (test-mode safety) + chat-precision diagnosis filed (2026-06-29)

- **New-account signups CLOSED by default** (`lib/signups.ts`, `app/_components/AuthForm.tsx`, `.env.example`): a shared test-mode link should not let strangers create accounts. Added `SIGNUPS_OPEN = process.env.NEXT_PUBLIC_ALLOW_SIGNUPS === 'true'` (default OFF, so it is active on the next deploy with no env change). `/signup` now renders a "Sign-ups are closed" card (no form, no Google button) and the "Create an account" link is hidden on `/login`; existing users still sign in with email/password or Google. Re-open at launch by setting `NEXT_PUBLIC_ALLOW_SIGNUPS=true`. This is an app-level guard only: OAuth/magic-link can still auto-create at the Supabase layer, so for an authoritative backstop also turn OFF "Allow new users to sign up" in the Supabase dashboard (Authentication, Sign In / Providers). Verified in preview: `/signup` shows the closed card; `/login` keeps the working form with no signup link.
- **Chat precision — diagnosis filed, build scheduled for Mon 2026-07-06.** Samuel's feedback (Slack): chat feels primitive, too chatty / lacks precision, "not connected to user model", jargon "no one understands" ("clarity broker", "Grounded Visionary", "insight synthesizer"). Read-only root-cause diagnosis (no build yet, per Will): (1) **verbosity is structural** — the chat system prompt `content/AI Model/System Prompt.md` §4.2/§4.3 defaults to a 5-element answer (Context/Insight/Options/Tradeoffs/Reflection) with required hedging phrases and no enforced brevity; `app/api/chat/route.ts:71` + `lib/ai.ts:54-68` set max_tokens 3000, reasoning off, no temperature/penalty tuning. (2) **user-model connection is conditional + localStorage-only** — `app/chat/page.tsx:184-190` builds `profileContext` from localStorage and `app/api/chat/route.ts:64-67` injects it only `if (ctx)`, so on a fresh browser/device or cleared storage the chat runs blind (matches Samuel's complaint); not server/Supabase-backed yet. (3) **jargon is LLM-generated**, not hardcoded — `lib/profile.ts` schema instruction `"title": "The <Evocative Two-Word Archetype>"` (+ Latin motto) and `lib/clarity.ts:40` "evocative" headline invent the clever labels, contradicting the prompt's own "no jargon, 8th-grade" tone rule; the hardcoded vocab (Realist/Doer/Organizer, Truth-Seeker/Builder, tool/family names like DesignSuite/Impact Pathways) is clean and stays. Full diagnosis + Monday fix plan in memory `align360-chat-precision.md`; scheduled task `align360-chat-precision-build` set for 2026-07-06 12:45 CST.
