...
export function fallbackProfile(scores: Scores, name: string): Profile {
  ...
  return {
    ...
    opportunity: {
      ...
      legacy: { note: 'These signals are incomplete, not wrong — read them as a baseline.', items: [{ score: 85, title: 'Roles aligned to your wiring', tag: `${w} · ${o}`, chips: ['—'], risk: '⚠ AI risk: connect the narrative engine for calibrated detail.' }] },
      ai: { title: 'The same profile. A different world.', note: 'AI concentrates value toward judgment and edge.', items: [{ score: 92, title: 'Edge-level work AI cannot replicate', chips: ['—'], why: 'Detailed AI-era signals generate when the narrative engine is connected.' }] },
    },
    aiEra: {
